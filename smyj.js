...
const $ = new Env('圣牧有机');
let status;
status = (status = ($.getval("smyjstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
const smyjurlArr = [], smyjhdArr = [],smyjcount = ''
let time = Math.round(Date.now() / 1000)
let smyjurl = $.getdata('smyjurl')
let smyjhd = $.getdata('smyjhd')
let smyjkey = '',uid=''
if ($.isNode()) {
...
!(async () => {
  ...
  } else {smyjurlArr.push($.getdata('smyjurl'))
    smyjhdArr.push($.getdata('smyjhd'))
    let smyjcount = ($.getval('smyjcount') || '1');
  for (let i = 2; i <= smyjcount; i++) {
    smyjurlArr.push($.getdata(`smyjurl${i}`))
    smyjhdArr.push($.getdata(`smyjhd${i}`))
  }
    console.log(`------------- 共${smyjhdArr.length}个账号-------------\n`)
      for (let i = 0; i < smyjhdArr.length; i++) {
        if (smyjhdArr[i]) {
          smyjurl = smyjurlArr[i];
          smyjhd = smyjhdArr[i];
          $.index = i + 1;
          console.log(`\n开始【圣牧有机签到${$.index}】`)
  ...
  .finally(() => $.done())
function smyjck() {
   if ($request.url.indexOf("checkin.json?") > -1) {
 const smyjurl = $request.url
  if(smyjurl)     $.setdata(smyjurl,`smyjurl${status}`)
    $.log(smyjurl)
  const smyjhd = JSON.stringify($request.headers)
        if(smyjhd)    $.setdata(smyjhd,`smyjhd${status}`)
$.log(smyjhd)
 $.msg($.name,"",`圣牧有机${status}token获取成功`)
  }
}
function smyjqd(timeout = 0) {
  return new Promise((resolve) => {
uid=smyjurl.match(/access_token=(\w+)/)[1]
let url = {
        url : `https://h5.youzan.com/wscump/checkin/checkin.json?checkin_id=13968&app_id=wx2f3e7fdb0cd94fab&kdt_id=45125701&access_token=${uid}`,
        headers : JSON.parse(smyjhd),
}
