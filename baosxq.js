...
function bsxqck() {
  ...
  const bsxqurl = $request.url
  if(bsxqurl)     $.setdata(bsxqurl,`bsxqurl${status}`)
    $.log(bsxqurl)
    const bsxqhd = JSON.stringify($request.headers)
        if(bsxqhd)    $.setdata(bsxqhd,`bsxqhd${status}`)
$.log(bsxqhd)
...
if(result.code == 1){
        console.log('宝石星球回执:失败🚫 '+result.msg+"别着急，还没出矿呢。")}
//$.msg("","","宝石星球已运行完毕")
        } catch (e) {
          //$.logErr(e, resp);
        } finally {
