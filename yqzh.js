...
const $ = new Env('悦趣赚')
let yqzh = $.getjson('yqzh',[{"url":"http://wechat04.yqzh.libcloud.cn/?parentId=77137&code=081mWb1w3XI86W2SG34w3T17x23mWb17&state=STATE","cks":{"vue_typescript_reader_autoReaderKey":"true","vue_typescript_serviceIMGurl":"http://file.sayyouideas.com/1614079826470ea76a7b3-1f77-40fb-a59d-61b4ca78ac1b.jpg","vue_typescript_reader_randomKey":"txrq69","vue_typescript_reader_token":"eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiI3OTU4NCIsIm5iY2hteWRwdDAwMyI6InR4cnE2OSIsImV4cCI6MTYxODEwNTA1NiwiaWF0IjoxNjE3MjQxMDU2fQ.L4L1-hi5fu-xMJHyxPqRKXWITjeuAz-gWVhCDQkBLD-45HXshWbCqbgDOwf39vBhOdmJARMFu6eqxRc5VNrKDA","vue_typescript_userbase":"{%22createTime%22:%22%22%2C%22headimgurl%22:%22https://thirdwx.qlogo.cn/mmopen/vi_32/Wd9icWRraTXVSy40Th1rl7PYXNfqicU3j5ycFoxbibXgtia0IUPqm5wDrkIoaWgAVUCJ56xLKA9db1bg1uH2TLEibqw/132%22%2C%22id%22:79584%2C%22idcode%22:%22LLkvC4%22%2C%22nickname%22:%22Ki%22%2C%22resettype%22:0}","vue_typescript_idCode_tipsKey":"true"},"hd":{"Accept-Encoding":"gzip,deflate","Connection":"keep-alive","Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8","Referer":"http://wechat04.yqzh.libcloud.cn/?parentId=77137&code=081mWb1w3XI86W2SG34w3T17x23mWb17&state=STATE","Host":"wechat04.yqzh.libcloud.cn","User-Agent":"Mozilla/5.0(iPhone;CPUiPhoneOS14_4likeMacOSX)AppleWebKit/605.1.15(KHTML,likeGecko)Mobile/15E148MicroMessenger/8.0.2(0x18000239)NetType/4GLanguage/zh_CN","Upgrade-Insecure-Requests":"1","Cache-Control":"max-age=0","Accept-Language":"zh-cn"},"id":79584}])
let needNotice = $.getval('yqzhNotice') == 'true'
let concurrency = ($.getval('yqzhConcurrency') || '1') - 0 // 并发执行任务的账号数，默单账号循环执行
  ...
  !(async () => {
    if (typeof $request !== "undefined") {
      ...
      for (let arr of execAcList) {
        ...
        msgInfo.push(rtList.map(ac => `【账号${ac.no}】任务执行完毕${ac.extMsg.length>0?'\n\t'+ac.extMsg.join('\n\t'):''}`).join('\n\n'))
      }
      if (msgInfo.length <= 0) {
        msgInfo.push(`暂无账号数据，请进入自动任务完成一次阅读任务抓取数据`)
      }
      if (needNotice) {
...
function execTask(ac, i) {
  return new Promise(async resolve => {
    try {
      await $.wait(i * 50)
      ac.extMsg.push(`当前文章单价：${(await getApi(`brushRuleApi/getBrushRuleInfo?timeplate=${new Date().getTime()}`, ac) || {unitPrice:'获取失败'})['unitPrice']}`)
      let info = await getApi(`withdrawApi/withdrawInfo?timeplate=${new Date().getTime()}`, ac)
      ac.extMsg.push(`任务前的收益：${info && info.allowWithdraw}`)
      // 获取文章
      let obj = (!execNo || (execNo.length == 0 || execNo.includes(ac.no))) ? await getApi(`nbadApi/info?timeplate=${new Date().getTime()}`, ac) : {message: '跳过阅读'}
      if (obj && obj.oid && obj.beforeMonitorUrlPre && obj.monitorUrlPre) {
        // 获取到文章数据，准备执行阅读任务
        let randomKey = ac.cks['vue_typescript_reader_randomKey']
        let count = 1
        do {
          $.log(`😄账号${ac.no} 获取${obj.type=='auth'?'检测':''}文章成功，开始阅读：${obj.oid}`)
          ...
          $.log(`🌝账号${ac.no} 等待${time}秒后提交第${count}个阅读任务：${bmsg}`)
          ...
          $.log(`🌝账号${ac.no} 提交第${count++}个阅读任务：${amsg}`)
          if (obj.type == 'auth') {
            $.log(`😂账号${ac.no} 提交的是鉴权任务(估计已黑)，等待6秒后尝试获取新任务`)
            await $.wait(6000)
          } else {
            await $.wait(500)
          }
      ...
      $.log(`🙃账号${ac.no} ${obj && obj.message}`)
      info = await getApi(`withdrawApi/withdrawInfo?timeplate=${new Date().getTime()}`, ac)
      ac.extMsg.push(`可提现的收益：${info && info.allowWithdraw}`)
      ac.extMsg.push(`平台的总收益：${info && info.sumAmount}`)
    } catch (e) {
...
async function getck() {
  ...
  if (url.indexOf("/?") > -1 && $request.headers['Cookie']) {
    ...
    if (userInfo.id && randomKey) {
      ...
      $.msg($.name, sub, `[账号${no+1}] ${status?'新增':'更新'}数据成功！`)
    } else {
