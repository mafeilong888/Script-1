/*
20210418
点击签到获取ck，每天0.88元，2元起提现，我已提现到账！
别充钱！！！！！！！！！！！！0撸为主！！！！
下载地址：https://raw.githubusercontent.com/xl2101200/-/main/tom/xyl.png
圈x
[task_local]
0 8 * * * https://raw.githubusercontent.com/xl2101200/-/main/xyl.js, tag=新亚链, enabled=true
[rewrite_local]
http://xinyaliyi.top/qiandao url script-request-body https://raw.githubusercontent.com/xl2101200/-/main/xyl.js
[MITM]
hostname = xinyaliyi.top
*/

const $ = new Env('新亚链');
let status;
status = (status = ($.getval("xylstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
const xylbodyArr = [], xylhdArr = [],xylurlArr = [],tycjcount = ''
let xylbody = $.getdata('xylbody')
let xylhd = $.getdata('xylhd')
...
!(async () => {
  ...
  } else {xylbodyArr.push($.getdata('xylbody'))
    xylhdArr.push($.getdata('xylhd'))
    xylurlArr.push($.getdata('xylurl'))
    let xylcount = ($.getval('xylcount') || '1');
  for (let i = 2; i <= xylcount; i++) {
    xylbodyArr.push($.getdata(`xylbody${i}`))
    xylurlArr.push($.getdata(`xylurl${i}`))
    xylhdArr.push($.getdata(`xylhd${i}`))
  }
    console.log(`------------- 共${xylhdArr.length}个账号-------------\n`)
      for (let i = 0; i < xylhdArr.length; i++) {
        if (xylhdArr[i]) {
         
          xylbody = xylbodyArr[i];
          xylhd = xylhdArr[i];
          xylhd = xylhdArr[i];
          $.index = i + 1;
          console.log(`\n开始【新亚链${$.index}】`)
...
function xylck() {
   if ($request.url.indexOf("qiandao") > -1 || $request.url.indexOf("collectFruit") > -1) {
 const xylbody = $request.body
  if(xylbody)     $.setdata(xylbody,`xylbody${status}`)
    $.log(xylbody)
const xylurl = $request.url
  if(xylurl)     $.setdata(xylurl,`xylurl${status}`)
    $.log(xylurl)
  const xylhd = JSON.stringify($request.headers)
        if(xylhd)    $.setdata(xylhd,`xylhd${status}`)
$.log(xylhd)
   $.msg($.name,"",'新亚链'+`${status}` +'headrs获取成功！')
  }
}

function xylqd(timeout = 0) {
  return new Promise((resolve) => {
let url = {
        url : `http://xinyaliyi.top/qiandao`,
        headers : JSON.parse(xylhd),
        body : xylbody,}
